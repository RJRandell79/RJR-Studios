...
<section class="container-fluid">
	<div class="container">
        ...
        while (have_posts()):
            ...
            $banner_src = wp_get_attachment_image_src(
                $banner_id,
                'medium_large'
            );
            $banner_srcset = wp_get_attachment_image_srcset(
                $banner_id,
                'medium_large'
            );
            $banner_alt = get_post_meta(
                $banner_id,
            ...
            $fullbanner_src = wp_get_attachment_image_src(
                $fullbanner_id,
                'full'
            );
            ?>
					<p class="metadata">In <span><?php echo $type; ?></span> / By <span>Rob Randell</span> on <span><?php echo the_time(
    'jS F Y'
); ?></span></p>
	                ...
	                <div class="project-carousel">
	                    <?php while (have_rows('screenshots')):
                         ...
                         $img_full_src = wp_get_attachment_image_src(
                             $img_full_id,
                             'full'
                         );
                         ?>
						<a data-fancybox="carousel" href="<?php echo $img_full_src[0]; ?>">
							<img src="<?php echo esc_url($img_src[0]); ?>" srcset="<?php echo esc_attr(
    $img_srcset
); ?>" sizes="(max-width: 600px) 100vw, 600px" alt="<?php echo $img_alt; ?>" />
						</a>
						<?php
                     endwhile; ?>
	                </div>
					...
					<a data-fancybox="banner" href="<?php echo $fullbanner_src[0]; ?>">
						<img src="<?php echo esc_url($banner_src[0]); ?>" srcset="<?php echo esc_attr(
    $banner_srcset
); ?>" sizes="(max-width: 600px) 100vw, 600px" alt="<?php echo $banner_alt; ?>" />
					</a>
         ...
         $endtag = end($posttags); ?>
	                <div class="tags clearfix">
	                    ...
	                    <ul class="clearfix">
	                        ...
	                        <li><a href="<?php echo get_tag_link(
                             $tag->term_id
                         ); ?>" title="<?php echo ucfirst(
    $tag->name
); ?>"><?php echo ucfirst($tag->name); ?></a><?php if ($tag != $endtag):
    echo ', ';
endif; ?></li>
	                        <?php endforeach; ?>
	                    </ul>
